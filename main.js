...
function stopServer(device) {
    if (devices[device].server) {
        devices[device].server.close();
    }
}
    ...
    socket.on('message', function (msg, rinfo) {
        if (msg.toString().match(/^(M-SEARCH) \* HTTP\/1.\d/)) {
            ...
            if (headers.man === '"ssdp:discover"') {
                ...
                for (let j = 0; j < devArray.length; j++) {
                    socket.send(
                        devices[devArray[j]].SSDP_RESPONSE,
                        0,
                        devices[devArray[j]].SSDP_RESPONSE.length,
                        rinfo.port,
                        rinfo.address,
                    );
                }
